...
class TitanBot extends Client {
  ...
  startWebServer() {
    ...
    // Keep the in-process limiter bounded when the public health endpoint is
    // scanned from many one-off addresses. The interval is unref'd so it can
    // never keep the bot alive during shutdown.
    this.webRateLimitCleanup?.();
    const cleanupInterval = Math.max(1_000, Math.min(windowMs, 60_000));
    ...
    cleanupTimer.unref?.();
    this.webRateLimitCleanup = () => clearInterval(cleanupTimer);

    app.get('/health', (req, res) => {
      const dbStatus = this.db?.getStatus?.() || { isDegraded: 'unknown' };
      const healthy = this.isReady() && !dbStatus.isDegraded;
      const status = {
