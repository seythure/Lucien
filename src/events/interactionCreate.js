      ...
      try {
        ...
        if (interaction.isChatInputCommand()) {
          try {
            ...
            // Economy commands mutate a shared account document. Serializing a
            // member's requests closes the duplicate-click/read-modify-write
            // window across all economy commands, not just individual ones.
            if (String(command.category).toLowerCase() === 'economy') {
              const accountLock = `economy-command:${interaction.guildId || 'dm'}:${interaction.user.id}`;
              await Mutex.runExclusive(accountLock, () => command.execute(interaction, guildConfig, client));
            } else {
