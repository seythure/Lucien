...
/**
 * Serializes operations for every affected account in a stable order. This
 * prevents the classic read-modify-write loss (and opposite-direction
 * transfer deadlock) inside a running bot process. PostgreSQL remains the
 * source of truth; this guard protects the application-level JSON updates.
 */
async function runWithEconomyLocks(guildId, userIds, task) {
  const lockKeys = [...new Set(userIds)]
    .filter(Boolean)
    .map((userId) => `economy:${guildId}:${userId}`)
    .sort();

  const acquireNext = async (index) => {
    if (index >= lockKeys.length) {
      return task();
    }
    return Mutex.runExclusive(lockKeys[index], () => acquireNext(index + 1));
  };
...
function lockEconomyMethod(methodName, getUserIds) {
  const original = EconomyService[methodName];
  EconomyService[methodName] = async function lockedEconomyMethod(...args) {
    const [client, guildId] = args;
    return runWithEconomyLocks(guildId, getUserIds(...args), () => original.apply(this, args));
  };
}

lockEconomyMethod('claimDaily', (_client, _guildId, userId) => [userId]);
lockEconomyMethod('transferMoney', (_client, _guildId, senderId, receiverId) => [senderId, receiverId]);
lockEconomyMethod('addMoney', (_client, _guildId, userId) => [userId]);
lockEconomyMethod('removeMoney', (_client, _guildId, userId) => [userId]);
lockEconomyMethod('depositToBank', (_client, _guildId, userId) => [userId]);
lockEconomyMethod('withdrawFromBank', (_client, _guildId, userId) => [userId]);
