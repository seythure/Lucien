...
const MAX_BLOCKED_ATTEMPT_ENTRIES = 10_000;
...
function recordBlockedAttempt(key, commandName, interaction, command, remainingMs) {
  ...
  if (!current && blockedAttemptStore.size >= MAX_BLOCKED_ATTEMPT_ENTRIES) {
    for (const [storedKey, storedValue] of blockedAttemptStore) {
      if (now - storedValue.windowStart > anomalyPolicy.windowMs) {
        blockedAttemptStore.delete(storedKey);
      }
    }
    while (blockedAttemptStore.size >= MAX_BLOCKED_ATTEMPT_ENTRIES) {
      const oldestKey = blockedAttemptStore.keys().next().value;
      if (oldestKey === undefined) break;
      blockedAttemptStore.delete(oldestKey);
    }
  }
